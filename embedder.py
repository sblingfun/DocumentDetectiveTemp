...
def convert_text_embedding(text_to_embed: str):
    ...
    return embedding_model.encode("search_document: " + text_to_embed)

def convert_query_embedding(query_to_embed: str):
    return embedding_model.encode("search_query: " + query_to_embed)
...
def create_and_store_embeddings(pages_and_chunks: list):
    ...
    idx = 0 
    for item in tqdm(pages_and_chunks):
        for entry in item["sentence_chunks"]:
            idx += 1       
            vector_db.insert_into_vector_db(PointStruct(id=idx,vector=convert_text_embedding(entry), payload = {
                "text": entry,
